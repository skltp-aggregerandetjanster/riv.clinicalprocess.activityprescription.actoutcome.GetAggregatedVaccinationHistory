...
@WebService(serviceName = "GetVaccinationHistoryResponderService", portName = "GetVaccinationHistoryResponderPort", targetNamespace = "urn:riv:clinicalprocess:activityprescription:actoutcome:GetVaccinationHistory:2:rivtabp21", name = "GetVaccinationHistoryInteraction")
public class GetAggregatedVaccinationHistoryTestProducer implements GetVaccinationHistoryResponderInterface {
	...
	public GetVaccinationHistoryResponseType getVaccinationHistory(String logicalAddress, GetVaccinationHistoryType request) {
		log.info("#### Virtual service for GetVaccinationHistory call the source system with logical address: {} and patinentId: {}", logicalAddress, request.getPatientId().getId());
		final GetVaccinationHistoryResponseType response = (GetVaccinationHistoryResponseType) testDb.processRequest(logicalAddress, request.getPatientId().getId());
		if(response == null) {
			return new GetVaccinationHistoryResponseType();
		}
		log.info("### Virtual service got {} in the reply from the source system with logical address: {} and patientId: {}", new Object[] {response.getVaccinationMedicalRecord().size(), logicalAddress, request.getPatientId().getId()});
		return response;
	}
}